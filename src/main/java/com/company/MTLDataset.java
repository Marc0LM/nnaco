...
public class MTLDataset {

    public static final int numFeatures=9;
    public static final int numLabels=12;
    public static final int numInstances=398;

    public List<DataSet> datasets;
    public ListDataSetIterator iterator;
    public MTLDataset(int batchSize, int seed, int foldidx, Boolean train) throws IOException {
        ...
        Files.lines(Paths.get("mpg_data")).forEach(s->{
            ...
            try {
                ...
                featureVector[id[0] * numFeatures] = vs.get(1).charAt(0) == '?' ? 1 / 512.f : Double.parseDouble(vs.get(1));
                featureVector[id[0] * numFeatures + 1] = vs.get(2).charAt(0) == '?' ? 1 / 512.f : Double.parseDouble(vs.get(2));
                featureVector[id[0] * numFeatures + 2] = vs.get(3).charAt(0) == '?' ? 1 / 512.f : Double.parseDouble(vs.get(3));
                featureVector[id[0] * numFeatures + 3] = vs.get(4).charAt(0) == '?' ? 1 / 512.f : Double.parseDouble(vs.get(4));
                featureVector[id[0] * numFeatures + 4] = vs.get(5).charAt(0) == '?' ? 1 / 512.f : Double.parseDouble(vs.get(5));
                featureVector[id[0] * numFeatures + 5] = vs.get(6).charAt(0) == '?' ? 1 / 512.f : Double.parseDouble(vs.get(6));
                switch (Integer.parseInt(vs.get(7).substring(0, 1))) {
                    case 1:
                ...
                labelVector[id[0] * numLabels] = Double.parseDouble(vs.get(0));
                for(int i=1;i<12;i++) {
                    labelVector[id[0] * numLabels + i] = featureVector[id[0] * numFeatures + i%7]+featureVector[id[0] * numFeatures + 1 + i%7];
                }
                id[0]++;
            }
            catch (Exception e){
